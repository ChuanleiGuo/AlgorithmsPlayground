...
public class FindNearestRepeatEntry {
    public static int findNearestRepetition(List<String> words) {
        Map<String, Integer> latestIdx = new HashMap<String, Integer>();
        Integer nearestDistance = Integer.MAX_VALUE;
        for (int i = 0; i < words.size(); i++) {
            if (latestIdx.containsKey(words.get(i))) {
                nearestDistance = Math.min(nearestDistance, i - latestIdx.get(words.get(i)));
            }
            latestIdx.put(words.get(i), i);
        }
