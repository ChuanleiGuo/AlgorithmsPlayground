...
public class SearchRealSquareRoot {
    public static double squareRoot(double x) {
        ...
        while (compare(left, right) == Ordering.SMALLER) {
            ...
            if (compare(squaredMid, x) == Ordering.EQUAL) {
                return mid;
            } else if (compare(squaredMid, x) == Ordering.LARGER) {
                right = mid;
            } else {
                left = mid;
            }
    ...
    public static void main(String[] args) {
        ...
        for (int times = 0; times < 100000; ++times) {
            ...
            System.out.println((res[0] = squareRoot(x)) + " " + (res[1] = Math.sqrt(x)));
            assert (compare(res[0], res[1]) == Ordering.EQUAL);
        }
    }
}
